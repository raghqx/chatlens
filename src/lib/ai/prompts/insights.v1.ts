...
export function buildOverview(digest: Digest): string {
  const p = digest.participants
    .map(
      (x) =>
        `- ${x.alias}: ${x.messages} messages (${x.sharePct}%), avg ${x.averageWords} words, median reply ${
          x.medianReplyMinutes === null ? 'n/a' : `${x.medianReplyMinutes} min`
        }, started ${x.conversationsStarted} conversations`,
    )
    .join('\n');
